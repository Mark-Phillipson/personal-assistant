...
def sum_business(input_path: str):
    total = Decimal('0')
    count = 0
...
def main(argv=None):
    ...
    p = argparse.ArgumentParser(description='Sum MoneyOut for Business Expense rows')
    ...
    count, total = sum_business(str(inp))
    out_lines = [f'Business Expense rows: {count}', f'Business Expense MoneyOut total: {total:.2f}']
    out_text = '\n'.join(out_lines)
    print(out_text)
    if args.output:
